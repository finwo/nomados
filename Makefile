...
dep/linux/arch/$(ARCH)/boot/bzImage: dep/linux
	...
	# Ensure google compute support
	(cd dep/linux ; echo "CONFIG_KVM_GUEST=y"                 >> .config)
	(cd dep/linux ; echo "CONFIG_KVM_CLOCK=y"                 >> .config)
	(cd dep/linux ; echo "CONFIG_VIRTIO_PCI=y"                >> .config)
	(cd dep/linux ; echo "CONFIG_SCSI_VIRTIO=y"               >> .config)
	(cd dep/linux ; echo "CONFIG_VIRTIO_NET=y"                >> .config)
	(cd dep/linux ; echo "CONFIG_PCI_MSI=y"                   >> .config)
	# Harden a tiny bit
	(cd dep/linux ; echo "CONFIG_STRICT_DEVMEM=y"             >> .config)
	(cd dep/linux ; echo "CONFIG_DEVKMEM=n"                   >> .config)
	(cd dep/linux ; echo "CONFIG_DEFAULT_MMAP_MIN_ADDR=65536" >> .config)
	(cd dep/linux ; echo "CONFIG_DEBUG_RODATA=y"              >> .config)
	(cd dep/linux ; echo "CONFIG_DEBUG_SET_MODULE_RONX=y"     >> .config)
	(cd dep/linux ; echo "CONFIG_CC_STACKPROTECTOR=y"         >> .config)
	(cd dep/linux ; echo "CONFIG_COMPAT_VDSO=n"               >> .config)
	(cd dep/linux ; echo "CONFIG_COMPAT_BRK=n"                >> .config)
	(cd dep/linux ; echo "CONFIG_SYN_COOKIES=y"               >> .config)
	(cd dep/linux ; echo "CONFIG_SECURITY_YAMA=y"             >> .config)
	(cd dep/linux ; echo "CONFIG_SECURITY_YAMA_STACKED=y"     >> .config)
	# Bootable
